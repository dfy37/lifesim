...
class TrajectoryEventMatcher:
    ...
    def process_trajectory(self, trajectory: List[POI_Event], user_profile: str, longterm_goal: str = '', max_n_events: int = 10, random_start_event: int = 10086) -> Tuple[List[Dict], str]:
        ...
        results = []
        traj = ''
        for i, e in enumerate(trajectory[:20]):
            traj += f'[{str(i)}]\n' + e.desc(sep='\n', keys_to_drop=['life_event', 'intent']) + '\n'

        PROMPT = (
            "You are provided with a user profile, a long-term goal, user trajectory and scenario themes.\n"
            f"You need to generate {max_n_events} events the user may experience corresponding to the user trajectory.\n"
            "For each event, you need to provide life_event and intent.\n"
            ...
            "```\n"
            "Where:\n"
            "- time: The time of the event in 'YYYY-MM-DD HH:MM:SS, Day' format. The same in user trajectory.\n"
            "- location: The location of the event. The same in user trajectory.\n"
            "- weather: a breif description of the weather during the event. The same in user trajectory.\n"
        ...
        assert len(parsed_results) >= max_n_events - 2, "Generated events are fewer than expected."
        events = []
