...
def build_simulator(on_turn_update, exp_name, config_path="config.yaml", assistant_model_name=None):
    ...
    analysis_m_cfg = cfg["models"].get("analysis_model", user_m_cfg)

    _user_model_name = os.path.basename(user_m_cfg["model_path"])
    _asst_model_name = assistant_model_name or os.path.basename(asst_m_cfg["model_path"])
    _analysis_model_name = os.path.basename(analysis_m_cfg["model_path"])

    user_model = load_model(
        model_name=_user_model_name,
    ...
    analysis_model = load_model(
        ...
        api_key=analysis_m_cfg["api_key"],
        model_path=analysis_m_cfg["model_path"],
        base_url=analysis_m_cfg["base_url"],
        vllmapi=analysis_m_cfg["vllmapi"],
    )
    ...
    user_retriever_cfg = build_retriever_config(f"user_memory_{exp_name}", 0, retriever_cfg)
    asst_retriever_cfg = build_retriever_config(f"assistant_memory_{exp_name}", 0, retriever_cfg)

    with open(preference_file) as f:
        preference_dims = json.load(f)
