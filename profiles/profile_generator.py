...
#         base_info = ", ".join(base_info_parts)
#         base_info_text = f"用户基本信息: {base_info}"

#         politics_econ_parts = []
#         if "income" not in keys_to_drop and self.income:
#             politics_econ_parts.append(f"收入水平为{self.income}")
#         if "employment" not in keys_to_drop and self.employment:
#             politics_econ_parts.append(f"{self.employment}")
#         politics_econ_text = ", ".join(politics_econ_parts)

#         body_state_text = f"当前身体状态: {self.body_state}" if "body_state" not in keys_to_drop and self.body_state else ""

#         sport_preferences_text = f"运动与饮食偏好: {self.sport_preferences}" if "sport_preferences" not in keys_to_drop and self.body_state else ""
        
#         personality_text = ""
#         if "personality" not in keys_to_drop and self.personality:
#             personality_text = "性格特征包括: " + "、".join(self.personality)
        
#         preferences_text = ""
#         if "preferences" not in keys_to_drop and self.preferences:
#             preferences_text = "交互偏好: " + " ".join(self.preferences)

#         conv_history_text = "用户交互历史: \n" + '\n'.join(self.conv_history)

#         parts = [base_info_text, politics_econ_text, body_state_text, sport_preferences_text, personality_text, preferences_text]
